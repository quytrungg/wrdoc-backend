...
class UserRegisterAPIView(KnoxLoginView):
    ...
    auth_backend_class = "apps.users.auth_backends.UsernameBackend"

    def post(self, request, *args, **kwargs):
        """Complete account creation process for user."""
        serializer = self.serializer_class(data=request.data)
        serializer.is_valid(raise_exception=True)
        registered_user = serializer.save()
        login(request, registered_user, self.auth_backend_class)
        return super().post(request, format=None)
